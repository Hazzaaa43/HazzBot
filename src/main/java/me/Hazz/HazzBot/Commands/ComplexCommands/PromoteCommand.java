        ...
        if (RankTester == 1) {
            try {
                ...
                if (role.equals("ace")) {
                    ...
                    guild.addRoleToMember(name, rank).queue();
                ...
                embed.addField("Command Successful", "`" + role.substring(0, 1).toUpperCase() + role.substring(1).toLowerCase() + "` has been added to `" + name.getUser().getAsTag() + "`", false);
                ...
                EmbedBuilder error = new EmbedBuilder();
                error.setColor(new Color(0xff0000));
                error.addField("Missing Arguments", "Correct Usage: `" + Info.PREFIX + "promote [User] [Role]`\nAllowed Roles:\nAce, Elite, Master, Legend, Champion, Omega, Elite4, Pokemaster", false);
                error.setFooter("Command ran by " + event.getMember().getUser().getAsTag(), event.getMember().getUser().getAvatarUrl());
                event.getChannel().sendMessage(error.build()).queue(msg -> msg.delete().queueAfter(5, TimeUnit.SECONDS));
            }
        } else {
